...
const GlobalSearch = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const empResults = employees.filter(e =>
      ...
      (e.grade || e.designation || '').toLowerCase().includes(q) ||
      (e.location || '').toLowerCase().includes(q) ||
      String(e.id || '').toLowerCase().includes(q)
    ).slice(0, 5).map(e => ({
      type: 'employee',
      label: e.name,
      sub: `${e.grade || e.designation || ''} · ${e.department || ''}`,
      data: e
    }));

    const polResults = POLICIES
    ...
    const navResults = NAV_ITEMS
      ...
      .slice(0, 3)
      .map(n => ({ type: 'nav', label: n.label, sub: n.sub, navId: n.id }));
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:99999, display:'flex', alignItems:'flex-start', justifyContent:'center', paddingTop:80, background:'rgba(0,0,0,0.35)', backdropFilter:'blur(4px)' }}
      ...
      <div style={{ background:'#ffffff', border:'1px solid rgba(16,93,169,0.3)', borderRadius:16, width:'100%', maxWidth:580, boxShadow:'0 24px 80px rgba(16,93,169,0.2)', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 18px', borderBottom:'1px solid #e2e8f0' }}>
          ...
          <input ref={inputRef} value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={onKey}
            placeholder="Search employees, departments, policies, pages..."
        ...
        <div style={{ maxHeight:420, overflowY:'auto' }}>
          ...
          {!query && (
            <div style={{ padding:'14px 18px', fontSize:'.82rem', color:'#94a3b8', fontFamily:"'DM Sans',sans-serif", lineHeight:1.8 }}>
              ...
              {['Anirudh Verma', 'IT Department', 'Leave Policy', 'HR', 'IFC Location', 'Holiday Calendar'].map(s => (
          ...
          {query && results.length === 0 && (
            <div style={{ padding:'28px 18px', textAlign:'center' }}>
              ...
              <div style={{ color:'#64748b', fontSize:'.9rem', fontFamily:"'DM Sans',sans-serif" }}>No results for "<strong>{query}</strong>"</div>
              <div style={{ color:'#94a3b8', fontSize:'.78rem', marginTop:4 }}>Try searching by name, ID, department, or location</div>
            </div>
          )}
              ...
              {results.map((r,i) => (
                <div key={i} onClick={()=>handleSelect(r)}
                  style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 18px', cursor:'pointer',
                    ...
                    borderLeft: i===selectedIdx ? '3px solid #105da9' : '3px solid transparent',
                    transition:'all .1s' }}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:'.72rem', color:'#64748b', fontFamily:"'DM Sans',sans-serif" }}>{r.sub}</div>
                  </div>
        ...
        <div style={{ padding:'8px 18px', borderTop:'1px solid #e2e8f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', gap:16, fontSize:'.65rem', color:'#94a3b8', fontFamily:"'DM Sans',sans-serif" }}>
